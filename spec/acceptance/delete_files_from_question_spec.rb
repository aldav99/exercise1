...
feature 'Delete files from question', %q{
  ...
  given(:attachments) {create_list(:attachment, 2, attachmentable: question) }
  ...
  scenario 'User delete many files from question', js: true do
    attachments
    sign_in(user)
    visit question_path(question)

    find('#attachments').first(:link, "Delete file").click
    click_link 'Delete file'
